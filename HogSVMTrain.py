...
def load_images(dirname, amout=9999):
    ...
    while img_name != '':  # 文件尾
        ...
        imr = cv2.imread(img_name)
        if imr is not None:
            img_list.append(imr)
        else:
            print("discard file:", img_name)
        img_name = file.readline()
        amout -= 1
...
# postive lst path
pos_list = load_images(r'D:\Programs\MyResearch\HOG_SVM\INRIAPerson2\96X160H96\Train\pos.lst')
# negative lst path
full_neg_lst = load_images(r'D:\Programs\MyResearch\HOG_SVM\INRIAPerson2\Train\neg.lst')
sample_neg(full_neg_lst, neg_list, [128, 64])
print("get pos list length:", len(pos_list))
print("get neg list length:", len(neg_list))
